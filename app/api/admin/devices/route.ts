...
export async function POST(request: NextRequest) {
  try {
    ...
    // Call the manageUserDevices mutation with the appropriate operation
    const response = await client.mutations.manageUserDevices({
      operation,
      ...params
    });
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to manage user devices' },
      { status: 500 }
    );
  }
}
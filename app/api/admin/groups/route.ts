...
export async function POST(request: NextRequest) {
  try {
    ...
    // Call the manageUserGroups mutation with the appropriate operation
    const response = await client.mutations.manageUserGroups({
      operation,
      ...params
    });
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to manage user groups' },
      { status: 500 }
    );
  }
}